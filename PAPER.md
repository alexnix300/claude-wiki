...
- **Agent-agnostic portability (partial).** The reference implementation targets Claude Code. The repository also ships a universal `AGENTS.md` variant compatible with ~15 agents including Codex, OpenCode, Cursor, Aider, goose, Zed, Warp, Gemini CLI, Jules, and Factory, plus agent-specific extensions for OpenCode's command system and Cursor's `.cursor/rules/` format. Two limitations remain: (a) cross-session autonomous capture is only automatic on Claude Code via its global `~/.claude/CLAUDE.md` mechanism — other agents read their schema files only when working inside the vault directory, meaning work done in unrelated project directories does not feed the wiki without explicit action; (b) full behavioral parity across agents has not been empirically verified — OpenCode's command argument binding, Cursor's always-apply rule invocation, and per-agent handling of `[[wikilinks]]` may diverge in practice. A truly neutral single-source schema with generated agent-specific adapters remains future work.
