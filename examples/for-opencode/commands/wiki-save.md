...
2. Check if a relevant page already exists (read `wiki/index.md` first, then check the target directory).
...
- **Read Before Edit** — if updating, read the page fully first
- **Minimal Page Surface** — if creating, include only sections needed now
- **Surgical Updates** — touch only the section affected by this save
